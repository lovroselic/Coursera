...
class FlowGraph:
    ...
    def __init__(self, n):
        ...
        self.demands = [0] * n
        self.sumT = 0
        self.conserve = [[0 for _ in range(n)] for _ in range(n)] 
...
def addST(GR):
    ...
    for i in range(source):
        ...
        if LOG: print(i, "to source", source, i, -GR.in_[i])
        GR.add_edge(i, sink, GR.out_[i], 0)
        if LOG: print(i, "to sink", i, sink, GR.out_[i])
        
        GR.sumT += GR.out_[i]
        #update
        GR.out_[source] += -GR.in_[i]
        GR.in_[sink] -= GR.out_[i]
...
#         GR.conserve[u][v] = min(GR.conserve[u][v], GR.demands[v])
# =============================================================================
        GR.conserve[u][v] = max(GR.conserve[u][v], 0)
        
        #print("conserve", conserve)
        #print("GR.conserve[u][v]", GR.conserve[u][v])
      
        #if LOG: print(u, "->", v, "Flow", GR.edges[edgeIndex].flow, "capacity",GR.edges[edgeIndex].capacity, "lbound",l)
        
        
        flow = l + flow
        #flow = l + flow - conserve
        #print(flow)
        flowByEdge.append(flow)
        allFlow += flow
    return allFlow, flowByEdge
...
allFlow, flowByEdge = GetFlow(GR.reads)
if allFlow == GR.sumT:
    print("YES")
    for f in flowByEdge:
        print(f)
