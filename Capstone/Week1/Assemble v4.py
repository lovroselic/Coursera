...
def Reconstruct(ReadMap, path):
    ...
    for i, node in enumerate(path):
        ...
        if LOG: print(node, "reads", ReadMap[node].read, "next->", nextNode)
        overlap = ReadMap[node].findOverlap(nextNode)
        if LOG: print("..overlap", overlap)
        text += ReadMap[nextNode].read[overlap:]
...
# =============================================================================
#     for i, node in enumerate(ReadMap):
#         if LOG: print(i, "read", node.read)
#         overlap = node.overlaps[0]
#         if LOG: print("..overlap", overlap.id, "weight", overlap.size)
# =============================================================================
...
#text = open("tests/111", "r", encoding="utf8")
